...
> decision remaining for Expero — whether (and how) to replace the
...
In Expero terms: instead of the human opening 5 terminals
(architect, planner, builder, verifier, critic) and piping state
through `.expero/docs/`, **one main agent** calls Task(subagent=architect)
and Task(subagent=builder) as part of a normal conversation. State
still flows through `.expero/docs/` — subagents write files the main
...
```
# Today (v1.x)
$ bash expero.sh start architect         # terminal 1
$ bash expero.sh start planner            # terminal 2
$ bash expero.sh start builder M0-001    # terminal 3
  ...
$ bash expero.sh restart                 # human checklist, close terminals
...
Replace `bash expero.sh start <role>` with
`.claude/agents/expero-<role>.md` subagent definitions. Add a main
"expero-orchestrator" agent that knows when to dispatch each role.
...
- Main orchestrator agent needs careful prompt engineering — it's
  the coordination layer Expero previously outsourced to humans.
...
Ship `.claude/agents/expero-<role>.md` alongside the existing
...
2. Pure B sacrifices the tool-agnostic positioning that's one of
   Expero's three differentiators (see README). That's the wrong
...
1. **Subagent definitions**: `.claude/agents/expero-<role>.md` × 8,
...
2. **`init` copies subagent definitions** into `.claude/agents/` in
   the generated project (sibling of `.expero/`). Claude Code picks
...
4. **Non-goal: no orchestrator agent yet.** Main agent can Task()
   ...
   expero-orchestrator agent).

### Phase-2 scope (the real UX leap)

- Orchestrator agent `.claude/agents/expero-orchestrator.md` that
...
- Stop signal → subagent dispatch: orchestrator reads
  `.expero/signals/<id>-NEEDS_ARCH_REVIEW.json` and
...
- `.claude/agents/expero-<role>.md` × 8: generated, ~40 lines each
...
Expero's "manual multi-terminal" model was right in 2024, shakier
