...
| §4.2 | Quality Gates          | 🟢 enforced | `expero.sh gate` 5/5 全部落地（artifacts_valid / adr_compliance / security_clean / ci_passes / test_coverage） |
| §4.3 | Stop Signal 与恢复     | 🟢 enforced | JSON 信号 + `status` 解析 + resolved/ 归档 + expero-orchestrator 自动按 type 派发 handler role |
