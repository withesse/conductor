...
`Conductor`（指挥）源于 SPEC 原有的术语——人类协调者就叫 Conductor。框架改名让这层自指关系浮现出来：

- 人类 Conductor 选定 Scenario、处理 Stop Signal、决定里程碑边界
- `conductor-orchestrator` subagent 在 Claude Code 会话中扮演相同角色：读 roadmap、派发 role subagent、interpret Critic verdict
- `conductor.sh` CLI 是跨工具的指挥——Codex / Gemini 等环境下同样履行协调职责

核心理念保持不变：**AI 辅助工程不是"AI 写代码"，而是"AI 写的代码能被工程化地验证和集成"**。框架的每一个设计选择都服务于这个目标——让 AI 产出的工作可重复、可审查、可验证。

> 历史：2026-04 之前项目名为 Expero Agents（拉丁语 `expero` = 通过实践验证）。v2.0.0 重命名为 Conductor，因为 v1.1 之后的架构重心从"脚手架工具"转向"多层协作指挥"，新名字更准确。`.expero/` → `.conductor/` 是唯一的 breaking change，详见 CHANGELOG v2.0.0。
...
| §4.2 | Quality Gates          | 🟢 enforced | `conductor.sh gate` 5/5 全部落地（artifacts_valid / adr_compliance / security_clean / ci_passes / test_coverage） |
| §4.3 | Stop Signal 与恢复     | 🟢 enforced | JSON 信号 + `status` 解析 + resolved/ 归档 + conductor-orchestrator 自动按 type 派发 handler role |
...
> **架构与扩展**：`roles/`、`scenarios/`、`schemas/` 是数据而非代码——新增 role / scenario / artifact schema 不需要改 `conductor.sh`。完整架构说明见 [docs/ARCHITECTURE.md](./docs/ARCHITECTURE.md)，扩展配方见 [docs/EXTENDING.md](./docs/EXTENDING.md)。
...
- 不试图替代产品决策（Conductor 不告诉你该做什么，只帮你更好地做你决定要做的事）
...
Conductor 建立在五个核心抽象之上。理解这五个抽象，就理解了整个框架。
...
**Milestone 的边界是 Agent 重启点**。完成一个 Milestone 必须关闭所有 agent，从文件系统重新读取状态启动。这一条看起来繁琐，实际是 Conductor 最重要的效率优化——它强制了状态的外化。
...
Conductor 提供 8 个内置 Scenario（下文详述），也支持自定义。
...
Conductor 定义 8 个角色，分为核心（必需）和扩展（按需）两类。

**核心角色（任何 Scenario 都启用）**

| Role | Tier | 核心职责 | Owns |
|------|------|----------|------|
| **Planner** | Execution | 维护 roadmap、协调任务流转 | `.conductor/docs/vision.md`, `.conductor/docs/roadmap.md` |
| **Architect** | Reasoning | 架构决策、ADR、技术方案审查 | `.conductor/docs/adr/*.md`, `.conductor/docs/gap-analysis.md` |
| **Builder** | Execution | 实现代码、编写测试、修复 CI | 代码文件 |
| **Verifier** | Template | 测试计划、覆盖率审查、CI 状态 | `.conductor/docs/specs/*-test-plan.md`, `.conductor/docs/ci-status.md` |
| **Critic** | Execution | 代码审查、ADR 合规检查 | `.conductor/docs/review/*.md` |

**扩展角色（特定 Scenario 启用）**

| Role | Tier | 启用场景 | Owns |
|------|------|----------|------|
| **Sentinel** | Reasoning | 涉及安全敏感领域 | `.conductor/docs/security/*.md` |
| **Scribe** | Execution | 需要对外文档 | `.conductor/docs/public/*.md`, `CHANGELOG.md` |
| **Archaeologist** | Reasoning | 遗留代码理解 | `.conductor/docs/legacy/*.md`, `.conductor/docs/reverse-adr/*.md` |
...
stages:
  - id: spec-drafting
    ...
    role: planner
    output: .conductor/docs/specs/<task-id>.md
    next: architect-review
    
  - id: architect-review
    role: architect
    input: .conductor/docs/specs/<task-id>.md
  ...
  - id: test-plan-drafting
    role: verifier
    input: .conductor/docs/specs/<task-id>.md
    output: .conductor/docs/specs/<task-id>-test-plan.md
    next: implementation
  ...
  - id: code-review
    role: critic
    input: git diff
    output: .conductor/docs/review/<task-id>.md
    next:
      - approved: completed
...
**为什么把框架文档放在 `.conductor/docs/`**

- **不污染项目根目录**：很多项目自己就有 `docs/`，Conductor 的文档独立存放避免冲突
- **隔离边界清晰**：`.conductor/` 下的所有文件都是 Conductor 管理的，Builder 和其他 agent 禁止修改 `.conductor/` 以外的项目既有文档
- **便于 gitignore 策略**：如果需要，整个 `.conductor/docs/` 可以被独立归档或排除
...
Conductor 提供 8 个内置 Scenario。每个 Scenario 预定义了启用的 Role 集、Workflow 模板和文档结构。
...
**特殊要求**
- 必须有 `.conductor/docs/adr/ADR-divergence.md` 定义 Class A/B 分歧策略
...
**产出**
- `.conductor/docs/public/api-reference.md`
- `.conductor/docs/public/quickstart.md`
- `.conductor/docs/public/architecture.md`
- `.conductor/docs/public/onboarding.md`
- `CHANGELOG.md`
...
**特殊文档**
```
.conductor/docs/contracts/
  <service-a>-to-<service-b>.md   # 服务间契约
.conductor/docs/e2e-matrix.md                # 端到端测试矩阵
```
...
Conductor 是工具无关的方法论。核心约束只有一条：**agent 必须通过文件系统通信**。
...
人类在 Conductor 中扮演 Conductor（指挥者）角色：
...
**缓解**：定期执行 `grep NEEDS_ _ROADMAP .conductor/docs/roadmap.md`，所有 NEEDS_ 标记必须由对应 Role 处理。
...
**Q1：Conductor 和普通的 AI 编码助手有什么区别？**

A：普通 AI 编码助手是一次性交互（你问，它答）。Conductor 是工程流程，让多个 AI agent 在规则约束下协同完成大型项目，产出可追溯、可审查。

**Q2：我的项目只有 3K 行代码，值得用 Conductor 吗？**

A：不值得。Conductor 的结构化成本在小项目上是负担。5K 行以下单 agent 更高效。

**Q3：我不用 Claude Code，用 Cursor / Cline / Continue，Conductor 还能用吗？**

A：能。Conductor 是方法论，不依赖特定工具。只要工具支持读写文件系统就可以。不同工具的差异只在"是否自动加载 CLAUDE.md"（多数工具不会，需要手动告知）。

**Q4：我能否只用部分 Role？**

A：可以。Scenario 定义了不同 Role 组合。你也可以自定义——最小可用组合是 `Planner + Builder`，但强烈建议至少加上 `Critic`。

**Q5：Conductor 能完全自动化吗？**

A：不能也不应该。Conductor（人类）的判断在三个时刻不可替代：Scenario 选择、Stop Signal 处理、里程碑决策。追求全自动是反模式。

**Q6：如何证明 Conductor 真的比单 agent 好？**
...
**Q9：Conductor 和 Cursor Composer / Cline 的 Plan Mode 有什么区别？**

A：Composer/Plan Mode 是单 agent 内的多阶段。Conductor 是多 agent 跨会话协作。前者解决"复杂任务分步执行"，后者解决"大型项目长期协作"。可以组合使用——每个 Role 内部可以用 Plan Mode，Role 之间用 Conductor。

**Q10：我想把 Conductor 用于非代码场景（写论文、做研究、产品设计）？**

A：框架本身是通用的，但现成的 Scenario 是代码导向的。你需要自定义 Scenario：重新定义 Role 的 Owns、Artifact 的 Schema、Workflow 的 Gate。

---

## 附录 A：从 Agent Team 迁移到 Conductor
...
Conductor 是一套方法论，不是软件。本文档以 CC0 协议发布，可自由复制、修改、商用。
