...
### 2.0.2 — Quality Gate 执行器（3/5 已在 v1.2 落地）

`expero.sh gate <name> [task-id]` 已可用。3 个基础 gate 在 v1.2 发布：

- [x] **`artifacts_valid`** — 包 `cmd_validate`（v1.2）
- [x] **`adr_compliance <task>`** — 检查 Critic 的 review 文件 Verdict=APPROVED（v1.2）
- [x] **`security_clean`** — 安全汇总无 CRITICAL 行（v1.2）
- [x] **`all [task]`** — 元 gate，依次跑全部适用 gate（v1.2）
- [ ] **`ci_passes`** — 需要 `config.yaml` 加 `ci_commands` schema 字段（外部接口变化，延后）
