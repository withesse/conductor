...
`expero.sh gate <name> [task-id]` 5/5 全部已发布：
...
- [x] `.claude/agents/expero-<role>.md` × 8（per-role tools + model tier）
- [x] init 复制到项目 `.claude/agents/`，Claude Code 自动识别
- [x] 字节级同步测试（T26v），与 regen-skills 相同模式

**Phase 2 — orchestrator** ✅ 已交付（Unreleased）
- [x] `.claude/agents/expero-orchestrator.md`：读 roadmap + signals，
