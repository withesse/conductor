...
### v1.2 — 基础加固（已发布，Unreleased → 待发 v1.1）
...
- [x] **Artifact Schema 校验器**：`expero.sh validate [path]` 覆盖 SPEC §5.2
      全部 7 种 artifact（v2.0.3 的独立形态；Gate 集成留给 v2.0.2）
...
> 数据层已在 v1.2 落地：signals 目录、JSON schema、`status` 解析。本节聚焦
> 调度与生命周期管理——v1.2 只完成了「读」，v2.0.1 完成「写与分派」。

- [x] `.expero/signals/<id>-<type>.json` 与 `status` 解析（v1.2）
- [ ] 每个信号按类型分派建议的 Role（Architect / Sentinel / Planner）
- [ ] 信号解决后归档到 `.expero/signals/resolved/`，保留审计轨迹
- [ ] Role prompt 自动引用未解决信号（而非等 Conductor 人工分派）
...
- `artifacts_valid` — 调用 v1.2 的 `validate` 作为 Gate 前置（v1.2 已就绪）

**动机**：让 SPEC §4.2 的 Gate 真正成为结构约束，而非 prompt 约定。

### 2.0.3 — Artifact Schema 校验器（已在 v1.2 落地）

> 独立命令形态已在 v1.2 提前交付。v2.0.3 本身关闭；其 Gate 集成转为 2.0.2
> 的 `artifacts_valid` Gate 条目。

- [x] `expero.sh validate [path]` 覆盖 ADR / RADR / Spec / Test-Plan / Review
      / Security Report / Security Summary（v1.2）
- [x] 校验失败退出非 0，per-file 报告缺失 section（v1.2）
- [ ] 接入 Quality Gate 执行链（归入 2.0.2）
