#!/bin/bash
# Conductor — Universal Bootstrap
#
# Usage:
#   bash conductor.sh init <project-name> <scenario>
#   bash conductor.sh start <role> [task-id]
...
_CONDUCTOR_SRC="${BASH_SOURCE[0]:-$0}"
if [ -n "$_CONDUCTOR_SRC" ] && [ -f "$_CONDUCTOR_SRC" ]; then
  CONDUCTOR_SCRIPT_PATH="$(cd "$(dirname -- "$_CONDUCTOR_SRC")" 2>/dev/null && pwd)/$(basename -- "$_CONDUCTOR_SRC")"
else
...
# .conductor/ (e.g. when users call `start` outside a project).
tier_for_role() {
  local tier
...
# ─────────────────────────────────────────
# init: initialize a new Conductor project
# ─────────────────────────────────────────
cmd_init() {
  ...
  # Script source already resolved at load time into CONDUCTOR_SCRIPT_PATH
  # (see top of file). We pass it to _gen_scripts for the in-project copy.
  local script_src=${CONDUCTOR_SCRIPT_PATH:-}
  ...
  info "Initializing Conductor project"
  ...
  local staging
  staging=$(mktemp -d "$parent/.conductor-init.XXXXXX")
  trap 'rm -rf -- "$staging"' EXIT

  mkdir -p "$staging/.conductor" "$staging/.conductor/docs/adr" "$staging/.conductor/docs/specs" "$staging/.conductor/docs/review" "$staging/.conductor/signals" "$staging/.conductor/signals/resolved"
  ...
  _gen_conductor_config "$scenario"
  ...
  echo ""
  ok "Conductor project initialized"
  echo ""
  echo "Next steps:"
  echo "  cd $project"
  ...
  local first_role
  first_role=$(_json_get_array "$target/.conductor/scenarios/$scenario.json" active_roles | head -1)
  echo "  bash conductor.sh start ${first_role:-architect}"
  echo "  bash conductor.sh status"
}
...
cmd_status() {
  ...
  local scenario
  scenario=$(awk '/^scenario:/{print $2; exit}' .conductor/config.yaml)
  echo "  Scenario: ${scenario:-unknown}"
  echo ""

  echo "Documents:"
  _count_files "ADR" ".conductor/docs/adr/*.md"
  _count_files "Specs" ".conductor/docs/specs/*.md" "-test-plan"
  _count_files "Test Plans" ".conductor/docs/specs/*-test-plan.md"
  _count_files "Reviews" ".conductor/docs/review/*.md"
  [ -d ".conductor/docs/security" ] && _count_files "Security Reports" ".conductor/docs/security/*.md"
  [ -d ".conductor/docs/public" ] && _count_files "Public Docs" ".conductor/docs/public/*.md"
  [ -d ".conductor/docs/legacy" ] && _count_files "Legacy Docs" ".conductor/docs/legacy/*.md"
  [ -d ".conductor/docs/reverse-adr" ] && _count_files "Reverse ADRs" ".conductor/docs/reverse-adr/*.md"
  echo ""

  if [ -f ".conductor/docs/roadmap.md" ]; then
    echo "Task Status:"
    ...
    todo=$(_count_matches_re   "\|[[:space:]]+todo[[:space:]]+\|"         .conductor/docs/roadmap.md)
    inprog=$(_count_matches_re "\|[[:space:]]+in-progress[[:space:]]+\|"  .conductor/docs/roadmap.md)
    done_=$(_count_matches_re  "\|[[:space:]]+completed[[:space:]]+\|"    .conductor/docs/roadmap.md)
    blocked=$(_count_matches_re "\|[[:space:]]+blocked[[:space:]]+\|"     .conductor/docs/roadmap.md)
    printf "  %-12s %s\n" "todo:" "$todo"
    printf "  %-12s %s\n" "in-progress:" "$inprog"
    printf "  %-12s %s\n" "completed:" "$done_"
    printf "  %-12s %s\n" "blocked:" "$blocked"
  ...
  arch_review=$(_count_matches "NEEDS_ARCH_REVIEW"        .conductor/docs/roadmap.md)
  spec_clar=$(_count_matches  "NEEDS_SPEC_CLARIFICATION"  .conductor/docs/roadmap.md)
  sec_review=$(_count_matches "NEEDS_SECURITY_REVIEW"     .conductor/docs/roadmap.md)
  printf "  %-28s %s\n" "NEEDS_ARCH_REVIEW:"        "$arch_review"
  printf "  %-28s %s\n" "NEEDS_SPEC_CLARIFICATION:" "$spec_clar"
  printf "  %-28s %s\n" "NEEDS_SECURITY_REVIEW:"    "$sec_review"

  # Structured signals: .conductor/signals/*.json, parsed without jq.
  # Format documented in .conductor/signals/README.md.
  ...
  if [ -d ".conductor/signals" ]; then
    ...
    for sig in .conductor/signals/*.json; do
      ...
      if [ -n "$sid" ] && [ -n "$stype" ] && [ -f .conductor/docs/roadmap.md ]; then
        if grep -qE -- "\|[[:space:]]*${sid}[[:space:]]*\|.*${stype}" .conductor/docs/roadmap.md 2>/dev/null; then
          dup_pairs=$((dup_pairs + 1))
        fi
      fi
    done
    # Count archived signals (resolved/ is an audit trail — signals are
    # moved here after resolution to keep the live directory focused on
    # what still needs attention).
    local archived_struct=0 asig
    if [ -d ".conductor/signals/resolved" ]; then
      for asig in .conductor/signals/resolved/*.json; do
        [ -e "$asig" ] || continue
        archived_struct=$((archived_struct + 1))
      done
    fi
    if [ "$unresolved_struct" -gt 0 ] || [ "$resolved_struct" -gt 0 ] || [ "$archived_struct" -gt 0 ]; then
      echo ""
      echo "Stop Signals (.conductor/signals/*.json):"
      # Dispatch hints: which role typically handles each signal type.
      # Shown only when that type has unresolved count > 0, to keep the
      # table compact when nothing is pending.
      _dispatch_row() {
        local label=$1 count=$2 role=$3
        if [ "$count" -gt 0 ] && [ -n "$role" ]; then
          printf "  %-28s %-3s → dispatch to: %s\n" "$label" "$count" "$role"
        else
          printf "  %-28s %s\n" "$label" "$count"
        fi
      }
      _dispatch_row "NEEDS_ARCH_REVIEW:"        "$s_unresolved_arch"    "architect"
      _dispatch_row "NEEDS_SPEC_CLARIFICATION:" "$s_unresolved_spec"    "planner"
      _dispatch_row "NEEDS_SECURITY_REVIEW:"    "$s_unresolved_sec"     "sentinel"
      _dispatch_row "BLOCKED_BY_*:"             "$s_unresolved_blocked" ""
      [ "$s_unresolved_other" -gt 0 ] && printf "  %-28s %s\n" "other:" "$s_unresolved_other"
      printf "  %-28s %s\n" "(resolved, in-place):"     "$resolved_struct"
      printf "  %-28s %s\n" "(archived in resolved/):"  "$archived_struct"
    fi
  else
    ...
    if [ "$total_text" -eq 0 ]; then
      echo "  (no structured signals; see .conductor/signals/README.md)"
    fi
  fi
...
cmd_restart() {
  ...
  local required=("CLAUDE.md" ".conductor/docs/roadmap.md" ".conductor/docs/ci-status.md")
  local all_ok=true
  ...
  text_signals=$(_count_matches "NEEDS_ARCH_REVIEW"       .conductor/docs/roadmap.md)
  text_signals=$((text_signals + $(_count_matches "NEEDS_SPEC_CLARIFICATION" .conductor/docs/roadmap.md)))
  text_signals=$((text_signals + $(_count_matches "NEEDS_SECURITY_REVIEW"    .conductor/docs/roadmap.md)))
  ...
  if [ "$total_pending" -gt 0 ]; then
    ...
    [ "$struct_signals" -gt 0 ] && echo "    .conductor/signals/*.json:     $struct_signals"
    echo "    Consider resolving before restarting. Run 'bash conductor.sh status' for details."
    echo ""
  fi
  ...
  echo "        conductor-orchestrator subagent to 'continue to next milestone'"
  echo ""
  ...
  if [ -n "$current_scen" ]; then
    while IFS= read -r r; do
      [ -z "$r" ] && continue
      case "$r" in
        critic|builder|verifier) echo "        bash conductor.sh start $r <task-id>" ;;
        *)                       echo "        bash conductor.sh start $r" ;;
      esac
    done < <(_active_roles_for_scenario "$current_scen")
  else
    echo "        bash conductor.sh start <role> [task-id]"
  fi
}

# Count unresolved structured signals (.conductor/signals/*.json with
# "resolved": false). Used by cmd_restart and cmd_status. Emits a single
# integer on stdout.
_count_unresolved_struct_signals() {
  local n=0 sig
  [ -d ".conductor/signals" ] || { echo 0; return; }
  for sig in .conductor/signals/*.json; do
    [ -e "$sig" ] || continue
    local resolved
...
cmd_validate() {
  ...
  if [ -n "$target" ]; then
    ...
    local p
    for p in \
      .conductor/docs/adr/*.md \
      .conductor/docs/reverse-adr/*.md \
      .conductor/docs/specs/*.md \
      .conductor/docs/review/*.md \
      .conductor/docs/security/*.md; do
      [ -e "$p" ] || continue
      files+=("$p")
    done
  fi
...
# fits into CI pipelines (`bash conductor.sh gate X && deploy`).
#
# Implemented in v1.x:
...
# Pass when every artifact under .conductor/docs/**/*.md that has a schema
...
_gate_adr_compliance() {
  local task_id=$1
  local review=".conductor/docs/review/$task_id.md"
  if [ ! -f "$review" ]; then
...
_gate_ci_passes() {
  local config=".conductor/config.yaml"
  if [ ! -f "$config" ]; then
...
_gate_security_clean() {
  local summary=".conductor/docs/security/summary.md"
  if [ ! -f "$summary" ]; then
    echo "  No security summary present — gate passes by default"
    echo "  (run Sentinel to populate .conductor/docs/security/summary.md)"
    return 0
  fi
...
# the project-local .conductor/ (created by init, so self-contained when
# conductor.sh is copied into a project), fall back to the source repo layout
# (directory that contains this script). Fails hard if neither is present.
_resource_root() {
  # 1. Current working directory is a project (has .conductor/roles/).
  ...
  local script_dir=""
  if [ -n "${CONDUCTOR_SCRIPT_PATH:-}" ]; then
    script_dir=$(dirname -- "$CONDUCTOR_SCRIPT_PATH")
  fi
  # 2. Script lives at the root of a generated project (copied conductor.sh
  #    next to .conductor/). Used by sub-inits launched from inside a
  #    detached project that lost the source repo.
  if [ -n "$script_dir" ] && [ -d "$script_dir/.conductor/roles" ]; then
    echo "$script_dir/.conductor"
    return
  fi
  # 3. Script lives in the source repo (roles/ and scenarios/ at the
  #    top level, no .conductor/ sibling).
  if [ -n "$script_dir" ] && [ -d "$script_dir/roles" ]; then
    echo "$script_dir"
    return
  fi
  err "Cannot locate roles/ (tried .conductor/roles, ${script_dir:-<unresolved>}/.conductor/roles, ${script_dir:-<unresolved>}/roles)"
  exit 1
}
...
# .conductor/scenarios/ (when running from inside a project that was already
...
_current_scenario() {
  [ -f ".conductor/config.yaml" ] || { echo ""; return; }
  awk '/^scenario:/{print $2; exit}' .conductor/config.yaml
}
...
Conductor project (scenario: **$scenario**).
...
_gen_roadmap() {
  ...
  cp "$template_abs" .conductor/docs/roadmap.md
}

_gen_ci_status() {
  cat > .conductor/docs/ci-status.md << 'EOF'
# CI Status
...
_gen_scripts() {
  ...
  # run `bash conductor.sh start <role>` without the source repo present.
  # Caller must pass the absolute path resolved BEFORE any cd (see cmd_init).
  local script_path=$1
  [ -n "$script_path" ] && [ -f "$script_path" ] || return 0

  if [ "$script_path" != "$(pwd)/conductor.sh" ]; then
    cp "$script_path" conductor.sh
    chmod +x conductor.sh
  fi

  # Copy sidecar resources into .conductor/ so the new project is itself a
  ...
  # (.conductor/roles/), so we try both.
  local src_root resource_src
  src_root=$(dirname -- "$script_path")
  if   [ -d "$src_root/roles" ];          then resource_src="$src_root"
  elif [ -d "$src_root/.conductor/roles" ];  then resource_src="$src_root/.conductor"
  else return 0
  fi

  mkdir -p .conductor/roles
  cp "$resource_src/roles"/*.md .conductor/roles/
  # Role metadata file — tier + descriptions for every role. Also needed
  # at runtime inside a detached project.
  [ -f "$resource_src/roles/_meta.json" ] && cp "$resource_src/roles/_meta.json" .conductor/roles/

  if [ -d "$resource_src/scenarios" ]; then
    mkdir -p .conductor/scenarios/roadmaps
    cp "$resource_src/scenarios"/*.json .conductor/scenarios/
    if [ -d "$resource_src/scenarios/roadmaps" ]; then
      cp "$resource_src/scenarios/roadmaps"/*.md .conductor/scenarios/roadmaps/
    fi
  fi

  if [ -d "$resource_src/schemas" ]; then
    mkdir -p .conductor/schemas
    cp "$resource_src/schemas"/*.json .conductor/schemas/
  fi

  # Claude Code subagent definitions — .claude/agents/conductor-*.md.
  ...
  # .claude/ into .conductor/ (it lives beside .conductor/ per the standard
  ...
  if [ -n "$subagent_src" ]; then
    ...
    local f
    for f in "$subagent_src"/conductor-*.md; do
...
_gen_signals_readme() {
  # Structured stop signals live in .conductor/signals/*.json (see cmd_status).
  # This README documents the schema so Conductor tooling and agents agree
  # on the contract without re-reading conductor.sh.
  cat > .conductor/signals/README.md << 'EOF'
# Stop Signals
...
file here. `bash conductor.sh status` scans this directory and groups
unresolved signals by type.

## File naming

`.conductor/signals/<task-id>-<type>.json`

Example: `.conductor/signals/M0-003-NEEDS_ARCH_REVIEW.json`
...
| `NEEDS_SECURITY_REVIEW` | sentinel | Security report filed under `.conductor/docs/security/` |
...
# ─────────────────────────────────────────
cmd_help() {
  echo "Conductor v$CONDUCTOR_VERSION"
  echo ""
  echo "Commands:"
...
Examples:
  ...
  bash conductor.sh status
  bash conductor.sh validate
  bash conductor.sh gate artifacts_valid
  bash conductor.sh gate all M0-001
  bash conductor.sh restart
...
Notes:
  ...
  - 'validate' checks all .conductor/docs/**/*.md against SPEC §5.2 schemas;
...
Documentation: https://github.com/withesse/conductor
EOF
}
...
# Dispatch
# ─────────────────────────────────────────
# Skip dispatch when sourced (lets test-conductor.sh call functions directly).
