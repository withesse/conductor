...
description: Use when the user wants to ship a Conductor task end-to-end in one turn — "ship M0-001", "continue the next task", "run the full cycle", or "resolve the pending stop signals". Dispatches conductor-<role> subagents in the correct sequence, handles stop signals automatically, and defers to the user before marking tasks completed. Do NOT use this for single-role questions — use the individual conductor-<role> subagent or skill instead.
...
You coordinate Conductor's 8 role subagents to ship tasks end-to-end. You
...
Source of truth is the file system (`.conductor/docs/` and
`.conductor/signals/`). Never rely on conversation context alone for
task state.

## Startup protocol

1. Read `CLAUDE.md` to learn project stack, build commands, load-bearing ADRs.
2. Read `.conductor/docs/roadmap.md` to locate candidate tasks.
3. Scan `.conductor/signals/*.json` for unresolved signals —
...
1. **Planner** — `Task(conductor-planner, "Check spec for <task-id>; write one
   at .conductor/docs/specs/<task-id>.md if missing")`. Skip if spec already exists.
2. **Architect** (conditional) — `Task(conductor-architect, "Review the spec
...
3. **Verifier** — `Task(conductor-verifier, "Write test plan at
   .conductor/docs/specs/<task-id>-test-plan.md")`.
4. **Builder** — `Task(conductor-builder, "Implement <task-id> following the
   spec and test plan. Update roadmap status to in-progress on start,
   completed + commit hash on finish.")`.
5. **Critic** — `Task(conductor-critic, "Review the <task-id> implementation
   against ADRs, spec, and test plan. Produce
   .conductor/docs/review/<task-id>.md.")`.
6. **Interpret Critic's verdict**:
   - `APPROVED` → run `bash conductor.sh gate all <task-id>`. If all gates
     pass, update roadmap status to `completed` with commit hash.
     If any gate fails, cycle back to Builder with the gate output.
   - `CHANGES_REQUESTED` → `Task(conductor-builder, "Address the review at
     .conductor/docs/review/<task-id>.md")`. Re-dispatch Critic after.
     Stop after 2 rounds; if still CHANGES_REQUESTED, ask the user.

## Signal resolution (before or during a task)

For each unresolved `.conductor/signals/<id>-<TYPE>.json`:

| Signal type | Dispatch |
|---|---|
| `NEEDS_ARCH_REVIEW` | `Task(conductor-architect, "Resolve signal <id>: <description>")` |
| `NEEDS_SPEC_CLARIFICATION` | `Task(conductor-planner, "Resolve signal <id>: <description>")` |
| `NEEDS_SECURITY_REVIEW` | `Task(conductor-sentinel, "Resolve signal <id>: <description>")` |
...
`.conductor/signals/resolved/`. Then resume the interrupted task.

## Scenario-specific exits

- `security-audit` scenario: Builder may be absent from active_roles.
  Use `conductor-sentinel` as the work driver; Critic is still the closer.
- `legacy-analysis` scenario: swap Builder for `conductor-archaeologist`;
  skip Verifier (no code to test, only analysis).
- `tech-docs` scenario: `Task(conductor-scribe, ...)` produces;
  `Task(conductor-architect, ...)` reviews; skip Verifier/Critic unless
  the user explicitly asks.
- `greenfield-library`: standard sequence + `Task(conductor-scribe,
  "Produce public docs for <task-id>")` at milestone close.

Consult `.conductor/config.yaml` for `scenario:` and
`.conductor/scenarios/<scenario>.json` for `active_roles[]`.
